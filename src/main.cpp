...
void print_help(const char *prog_name) {
    std::cout << "Usage: " << prog_name << " [OPTIONS] [IDs/RANGES]\n"
              ...
              << "Options:\n"
              << "  -o, --output MODE      Output mode: bxcan_f0, bxcan_f4, fdcan_g0, fdcan_h7\n"
...
bool canfilter_cli(int argc, char *argv[]) {
    ...
    if (output_mode == "bxcan_f0") {
        filter = std::make_unique<canfilter_bxcan_f0>();
    } else if (output_mode == "bxcan_f4") {
        filter = std::make_unique<canfilter_bxcan_f4>();
